...
<script>
    ...
    function signPetition(petition) {
        var data = {
            id:petition,
            _token: '{{ csrf_token() }}'
        }
    ...
    function autoDebit(action) {
        ...
        if (action == 'true') {
            @if($data->judge?->work_number)
                ...
                'value="{{ $first ? ($data->judge->work_number .' ('.$data->id.')') : $data->contract_name }}">' +
//            '<select name="bank_id" id="bank_id" class="form-control"><option value="1">Жорий</option><option value="2">APEXBANK</option></select>' +
                ...
                confirm: {
                    ...
                    action: function () {
                        ...
                        $.ajax({
                            ...
                            data: {_token: "{{ csrf_token() }}", action: action, contract_name: contract_name, bank_id: bank_id},
                            method: "GET",
                            success: function (data) {
                                $.alert({
                                    title: data.status,
    ...
    function saveAutotransportInfo() {
        ...
        console.log(form);
        $.ajax({
            url: "{{ asset('contracts/autotransport-info/'.$data->id) }}",
            method: "POST",
            data: form,
            success: function (data) {
                // message content
    ...
    function reloadPage() {
        fetch('/contracts/calc-residue/'+'{{ $data->id }}');
        ...
        <div class="col-md-6">
            <div class="card" id="user">
                ...
                <div class="card-body">
                    ...
                    <table class="table table-striped single-contract">
                        <tbody>
                        <tr>
                            <th>Иш туркуми</th>
                                ...
                                <form id="petition_{{ $petition->id }}">
                                    ...
                                    <div class="d-flex align-items-end justify-content-between">
                                        ...
                                        @if($petition->status == null)
                                            <button type="button" onclick="signPetition('{{ $petition->id }}')" class="btn btn-info">Имзолашга</button>
                                        @elseif($petition->status == 'new')
                                            <button type="button" class="btn btn-soft-secondary">Имзолашда</button>
                                        @else
                                            <button type="button" class="btn btn-soft-info">Имзоланди</button>
                                        @endif
                    ...
                    <div class="card">
                        ...
                        <div class="card-body" id="auto-info">
                            <form id="auto_information">
                                <table class="table table-sm single-contract">
                                    <tbody>
                                    <tr>
                                        <th>Русуми</th>
